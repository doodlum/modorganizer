...
cd -- "$instance_root"
mo2_command=(env QT_QPA_PLATFORM=windows:nowmpointer MO2_FRONTEND_HOST=1 "$proton_command" run "$instance_root/ModOrganizer.exe")
if [[ -n "$runtime_command" ]]; then
    exec "$runtime_command" -- "${mo2_command[@]}"
fi
exec "${mo2_command[@]}"
