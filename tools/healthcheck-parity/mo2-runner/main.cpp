...
QJsonArray entriesToJson(const QList<IssueEntry>& entries)
{
  ...
  for (const IssueEntry& entry : entries) {
    ...
    object.insert(QStringLiteral("severity"), issueSeverityToString(entry.severity));
...
int main(int argc, char* argv[])
{
  ...
  for (const QJsonValue& value :
    ...
    installed.fileVersionUid =
        object.value(QStringLiteral("fileVersionUid")).toString();
  ...
  for (const QJsonValue& value :
       fixture.value(QStringLiteral("candidates")).toArray()) {
  ...
  for (const QJsonValue& value :
       fixture.value(QStringLiteral("modDetails")).toArray()) {
  ...
  for (auto it = hydration.constBegin(); it != hydration.constEnd(); ++it) {
    g_hydrationRaw.insert(
        it.key(), it.value().toObject().value(QStringLiteral("file")).toObject());
  }
  ...
  const HydrateFile hydrate =
      [&](const QString& fileUID) -> std::optional<HydratedFile> {
    ...
    if (entry.value(QStringLiteral("kind")).toString() == QLatin1String("downloaded")) {
      ...
      hydrated.downloaded.fileUID =
          fileObject.value(QStringLiteral("fileUID")).toString();
      hydrated.downloaded.modUID =
          fileObject.value(QStringLiteral("modUID")).toString();
      hydrated.downloaded.modName =
      ...
      hydrated.downloaded.version =
          fileObject.value(QStringLiteral("version")).toString();
  ...
  QJsonObject out;
  out.insert(QStringLiteral("fixture"),
             fixture.value(QStringLiteral("name")).toString());
  out.insert(QStringLiteral("report"), reportToJson(report));
  out.insert(QStringLiteral("metadata"), metadataToJson(metadata));
  out.insert(QStringLiteral("entries"), entriesToJson(buildEntries(metadata, hidden)));
